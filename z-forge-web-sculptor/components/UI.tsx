...
export const UI = () => {
  ...
  const setSmartRetopology = useStore(state => state.setSmartRetopology);
  const blendStrength = useStore(state => state.blendStrength);
  const setBlendStrength = useStore(state => state.setBlendStrength);
        ...
        <div className="flex items-center gap-4">
            <div className="flex flex-col gap-1">
                ...
                <input 
                    ...
                    className="w-24 h-2 bg-zinc-700 rounded-lg appearance-none cursor-pointer accent-indigo-500"
                />
            </div>

            <div className="flex flex-col gap-1">
                <label className="text-[10px] uppercase text-zinc-500 font-semibold tracking-wider flex items-center gap-1">
                    <Blend size={10} /> Blend
                </label>
                <input 
                    type="range" 
                    ...
                    className="w-24 h-2 bg-zinc-700 rounded-lg appearance-none cursor-pointer accent-pink-500"
                    title="Mesh Fusion Blend Strength"
                />
            </div>
            
            <button 
                onClick={() => setSmartRetopology(!smartRetopology)}
                className={`p-2 rounded-lg border transition-all ${smartRetopology ? 'border-amber-500 bg-amber-500/10 text-amber-400' : 'border-zinc-700 text-zinc-500 hover:text-zinc-300'}`}
                title="Smart Auto-Retopology (Smooth)"
            >
                <Sparkles size={18} />
            </button>
